...
{
    public interface INativeStdScrWideStr
    {
    ...
    internal class NativeStdScrWideStr<TWideStr, TSmallStr> : NativeWideStrBase<TWideStr, TSmallStr>, INativeStdScrWideStr, IDisposable
        where TWideStr : unmanaged
        where TSmallStr : unmanaged
    {
        private Memory<TWideStr> input;
        private MemoryHandle inputHandle;

        //TODO: thread safety around input
        public NativeStdScrWideStr()
        {
            this.input = new Memory<TWideStr>(new TWideStr[1]);
        ...
        public void innwstr(out string wstr, int count, out int read)
        {
            unsafe
            {
                ...
                read = NCursesException.Verify(this.Wrapper.innwstr(ref MarshalString(strPtr, count, out Span<TWideStr> span), count), "innwstr");
        ...
        public void inwstr(out string wstr)
        {
            unsafe
            {
                ...
                NCursesException.Verify(this.Wrapper.inwstr(ref MarshalString(strPtr, Constants.MAX_STRING_LENGTH, out Span<TWideStr> span)), "inwstr");
                wstr = ReadString(ref span);
            }
        }
        ...
        {
            //zero input
            TWideStr* wideCh = stackalloc TWideStr[1];
            this.input.Span[0] = wideCh[0];

            return VerifyInput("get_wch", 
                this.Wrapper.get_wch(ref Unsafe.AsRef<TWideStr>(this.inputHandle.Pointer)), 
                this.input.Span, 
                out wch, 
                out key);
        }
        ...
        public void getn_wstr(out string wstr, int n)
        {
            unsafe
            {
                ...
                NCursesException.Verify(this.Wrapper.get_wstr(ref MarshalString(strPtr, n, out Span<TWideStr> span)), "get_wstr");
                wstr = ReadString(ref span);
            }
        }
        ...
        public unsafe bool mvget_wch(int y, int x, out char wch, out Key key)
        {
            ...
            return VerifyInput("mvget_wch", this.Wrapper.mvget_wch(y, x, ref Unsafe.AsRef<TWideStr>(this.inputHandle.Pointer)),
                this.input.Span,
                out wch,
                out key);
        }
        ...
        public void mvget_wstr(int y, int x, out string wstr)
        {
            ...
            {
                TWideStr* strPtr = stackalloc TWideStr[Constants.MAX_STRING_LENGTH];
                NCursesException.Verify(this.Wrapper.mvget_wstr(y, x, ref MarshalString(strPtr, Constants.MAX_STRING_LENGTH, out Span<TWideStr> span)), "mvget_wstr");
                wstr = ReadString(ref span);
            }
        }
        ...
        public void mvgetn_wstr(int y, int x, out string wstr, int n)
        {
            ...
            {
                TWideStr* strPtr = stackalloc TWideStr[n];
                NCursesException.Verify(this.Wrapper.mvgetn_wstr(y, x, ref MarshalString(strPtr, n, out Span<TWideStr> span), n), "mvgetn_wstr");
                wstr = ReadString(ref span);
            }
        }
        ...
        public void mvinwstr(int y, int x, out string wstr)
        {
            ...
            {
                TWideStr* strPtr = stackalloc TWideStr[Constants.MAX_STRING_LENGTH];
                NCursesException.Verify(this.Wrapper.mvinwstr(y, x, ref MarshalString(strPtr, Constants.MAX_STRING_LENGTH, out Span<TWideStr> span)), "mvinwstr");
                wstr = ReadString(ref span);
            }
        }
